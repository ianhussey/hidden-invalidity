...
```{r}
save(dist_results, file = "temp/dist_results.RData")
#load("temp/dist_results.RData")
```
...
# remove redundant objects to keep the environment tidy
rm(bidr_dist, icat_dist, nfcc_dist, soc_dist)

```

```{r}
save(dist_results_2, file = "temp/dist_results_2.RData")
#load("temp/dist_results_2.RData")
```
...
```{r}
save(consistency_results, file = "temp/consistency_results.RData")
#load("temp/consistency_results.RData")
```
...
```{r}
save(dependability_results, file = "temp/dependability_results.RData")
#load("temp/dependability_results.RData")
```
...
```{r}
save(stability_results, file = "temp/stability_results.RData")
#load("temp/stability_results.RData")
```
...
cfi_fit_results <- 
  ...
  mutate(pvalue = ifelse(pvalue < .0001, "< .0001", as.character(round(pvalue, 4))),
         `chisq/df`    = round(chisq/df, 1),
         `Model fit` = ifelse(srmr <= .09 & cfi >= .95 & tli >= .95 & rmsea <= .06, "Good",
                              ifelse(srmr <= .09 & (cfi >= .95 | tli >= .95 | rmsea <= .06), "Poor", "Mixed")),
         `Model fit` = fct_relevel(`Model fit`, "Good", "Mixed", "Poor")) %>%
  round_df(3) %>%
...
mi_via_cutoffs <- function(data, model, scale, group) {
  ...
  fit_indices <- results$fit.indices %>%
    ...
    mutate(MI_config = ifelse(srmr <= .09 & cfi.scaled >= .95 & tli.scaled >= .95 & rmsea.scaled <= .06, "Good",
                              ifelse(srmr <= .09 & (cfi.scaled >= .95 | tli.scaled >= .95 | rmsea.scaled <= .06), "Poor", "Mixed"))) %>%
    round_df(3) %>%
    mutate(Scale = scale,
           group = group)
...
# WRITE THESE OBJECTS TO DISK HERE AS THE FULL MI RESULTS 




# THEN SOME RESHAPED NECESSARY TO GET SUMMARIES ACROSS TESTS

#filter(test == "fit_configural") %>%
...
```{r}
save(MI_combined_temp, file = "temp/MI_combined_full.RData")
write_csv(MI_combined_temp, "temp/MI_combined_full.csv", na = "")
#load("temp/MI_combined_full.RData")

save(MI_combined, file = "temp/MI_combined.RData")
#load("temp/MI_combined.RData")
```
...
# mi tests cannot be run on ordinal data when there are variables that have no occurances of some response options. Running mi_via_cutoffs() belwo diagnoses such issues, down to the item and response option that is missing.
...
combined_split_scales <- cfi_fit_results %>%
...
combined_results <- 
  ...
  dplyr::rename(`Changes from original scale` = Changes,
                `α` = alpha,
                `α fit` = alpha_fit,
                `α lower` = alpha_ci_lwr,
                `α upper` = alpha_ci_upr,
                `ω_t` = omega_t,
                `ω_t fit` = omega_t_fit,
                `ω_t_lwr` = omega_t_ci_lwr,
                `ω_t_upr` = omega_t_ci_upr,
                `ω_h` = omega_h,
                `ω_h fit` = omega_h_fit,
                `ω_h_lwr` = omega_h_ci_lwr,
                `ω_h_upr` = omega_h_ci_upr,
                `χ^2` = chisq,
                `χ^2/df` = `chisq/df`,
                `RMSEA lower` = lower,
                `RMSEA upper` = upper,
